...
[![GoDoc](https://img.shields.io/badge/godoc-reference-blue.svg)](http://godoc.org/github.com/gordanet/dnsseeder)

DNSSeeder exposes a list of known peers to any new peer joining the Gord network via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the gord node
...
disk in a json file, so on subsequent start ups the gord node specified with
...
- Launch a gor node for the DNSSeeder to connect to
...
```bash
$ git clone https://github.com/gordanet/dnsseeder $GOPATH/src/github.com/gordanet/dnsseeder
$ cd $GOPATH/src/github.com/gordanet/dnsseeder
$ go install . 
```
...
To create a working set-up where the DNSSeeder can provide IPs to gord instances, set the following DNS records:
```
NAME                        TYPE        VALUE
----                        ----        -----
